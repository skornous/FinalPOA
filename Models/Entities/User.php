...
class User extends Entity {
	...
	public function isPartOf($teamLookup) {
		...
		if($this->teams === null || empty($this->teams)) { return false; }

		// what to compare to ?
		if(is_int($teamLookup)) {
			$searchID = 'id';
		} else if (is_string($teamLookup)) {
			$searchID = 'name';
		} else {
			return false;
		}
		...
		foreach ($this->teams as $team) {
			...
			if($team['id'] == 0 || $team['name'] == "Admin") { return true; }
	...
	public function extractTeamsIds() {
		if ($this->teams === null || empty($this->teams)) { return array(); }
		$teamsIds = array();
		foreach ($this->teams as $team) {
			$teamsIds[] = $team['id'];
		}
	...
	public function can($action = null, $object = null) {
		...
		if(is_null($action) || is_null($object)) { return false; }
		// does the user have all the existing rights ?
		if(array_key_exists("all", $this->auths) && in_array("all", $this->auths["all"])) { return true; }
		// does the user have the right to do that action on any objects ?
		if(array_key_exists("all", $this->auths) && in_array($action, $this->auths["all"])) { return true; }
		// does the user have all rights on the given object ?
		if(array_key_exists($object, $this->auths) && in_array("all", $this->auths[$object])) { return true; }
	...
	public function setDeleted($deleted) { $this->deleted = $deleted; return $this;}

	public function getPassword() { return $this->password; }
	public function setPassword($password) { $this->password = md5($password); return $this;}

	public function getTeams() { return $this->teams; }
	public function setTeams($teams) { $this->teams = $teams; return $this;}

	public function getAuths() { return $this->auths; }
	public function setAuths($auths) { $this->auths = $auths; return $this;}
}