...
class ScriptController extends AppController {
	...
	public function getNbScriptDatas($saving = true) {
		if((isset($_GET) && isset($_GET["month"])) || (isset($_POST) && isset($_POST["month"]))) {
			...
			$currMonth = array("month" => date("Y-m-t", strtotime($date)));
			$scripts = array(
			...
			if (!$saving) {
				$monthMinus12 = array("endMonth" => date("Y-m-t", strtotime($date)));
			...
			$scriptDatas = array("view" => "data.datas", "datas" => $datas);
		} else {
			$scriptDatas = array("view" => "errors.scriptNoDate", "datas" => null);
		}
	...
	protected function run($script, $variables = array()) {
