<?php
	...
	class UserController extends AppController {
				...
				if (strlen($datas["newpassword1"]) >= 5) {
					if ($datas["newpassword1"] === $datas["newpassword2"]) { // new password matches it's verification
						if ($this->User->checkPswd($datas["userID"], $datas["oldpassword"])) { // old pswd is correct
							...
							$state = ["danger" => "DOLDPSWD"];
						}
					} else {
						$state = ["danger" => "DNEWPSWD"];
					}
				} else {
					$state = ["danger" => "PSWDRULES"];
				}
			} else {
				$state = ["danger" => "SUSERDELETE"];
			}
		...
		public function changePasswordForm() {
			...
			$this->render("user.changePswd");
		}

		// -- Verifiers -- //
		private function verifyCreateForm($form = []) {

			$requiredFields = ["firstname", "lastname", "email", "login", "password"];
		...
		private function verifyModifyForm($form = []) {
			...
			$requiredFields = ["id", "firstname", "lastname", "email", "login"];
