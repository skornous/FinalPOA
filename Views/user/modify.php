<?php if(isset($formDatas) && !isset($user)) $user = $formDatas; ?>
<div class="container">
	<ol class="breadcrumb">
		<li><a href="<?php echo BASE_URL . '/users';?>"><?php echo $lang->USERLIST;?></a></li>
		<li><a href="<?php echo BASE_URL . '/user/show/' . $user->id;?>"><?php echo $lang->USER . " - " . $user->login; ?></a></li>
	...
	<div class="jumbotron row">
		<form method="POST" name="form_update" action="<?php echo BASE_URL; ?>/users/<?php echo $user->id; ?>">
			<div role="form-group" role="form">
				...
				<div class="col-sm-10 form-group">
					<select multiple="multiple" class="form-control" name="teams[]">
						<?php foreach ($teams as $team): ?>
							<option value="<?php echo $team["id"]; ?>"<?php if ($user->isStrictlyPartOf($team["name"])) echo ' selected="selected"'; ?>><?php echo $team["name"]; ?></option>
						<?php endforeach; ?>
					</select>
				</div>
				...
				<div class="col-sm-10 form-group">
					<input type="checkbox" name="active" <?php if($user->active){ echo'checked="checked"';}else{}?> > 
				</div>
